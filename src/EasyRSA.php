<?php
namespace Lyhiving\EasyRSA;

// PHPSecLib:
use Lyhiving\EasyRSA\Exception\InvalidKeyException;
use \phpseclib\Crypt\RSA;
// defuse/php-encryption:
use \Lyhiving\ConstantTime\Base64;
use \Defuse\Crypto\Key;
use \Defuse\Crypto\Crypto;
// Typed Exceptions:
use \Lyhiving\EasyRSA\Exception\InvalidChecksumException;
use \Lyhiving\EasyRSA\Exception\InvalidCiphertextException;
...
class EasyRSA implements EasyRSAInterface
{
    ...
    public static function decrypt($ciphertext, PrivateKey $rsaPrivateKey)
    {
        ...
        if (\count($split) == 1) {//JS encode
            $split[0] = str_replace(array(" ", '-', '_'), array("+", '+', '/'), $split[0]);
            foreach (str_split(base64_decode($split[0]), 128) as $chunk) {
                $res = openssl_private_decrypt($chunk, $decryptData, $rsaPrivateKey->getKey());
                if (!$res) {
                    return false;
                }
                $crypto .= $decryptData;
            }
            $crypto = str_replace('"', '', $crypto);
            return $crypto;
        }
        if (\count($split) !== 4) {
